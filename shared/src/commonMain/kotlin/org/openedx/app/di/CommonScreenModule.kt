...
val commonScreenModule = module {
    ...
    viewModel { (courseId: String?, infoType: String?, authCode: String) ->
        SignInViewModel(get(), get(), get(), get(), get(), get(), get(), get(), get(), get(), get(), get(), courseId, infoType, authCode)
    }
    ...
    viewModel { (openTab: String) -> LearnViewModel(openTab, get(), get(), get()) }
    ...
    viewModel { (querySearch: String) ->
        WebViewDiscoveryViewModel(
            querySearch = querySearch, appData = get(), config = get(),
            networkConnection = get(), corePreferences = get(), analytics = get(),
            resourceManager = get(),
        )
    }
    ...
    viewModel { ProfileViewModel(interactor = get(), resourceManager = get(), notifier = get(), analytics = get()) }
    ...
    viewModel { (qualityType: String) -> VideoQualityViewModel(qualityType, get(), get(), get(), get()) }
    ...
    viewModel { ManageAccountViewModel(interactor = get(), resourceManager = get(), notifier = get(), analytics = get()) }
    ...
    viewModel { (pathId: String, infoType: String) ->
        CourseInfoViewModel(pathId, infoType, get(), get(), get(), get(), get(), get(), get(), get())
    }
    viewModel { (courseId: String) ->
        CourseDetailsViewModel(courseId, get(), get(), get(), get(), get(), get(), get(), get())
    }
    viewModel { (courseId: String, courseTitle: String, resumeBlockId: String) ->
        CourseContainerViewModel(courseId, courseTitle, resumeBlockId, get(), get(), get(), get(), get(), get(), get(), get(), get())
    }
    viewModel { (courseId: String, courseTitle: String) ->
        CourseContentAllViewModel(courseId, courseTitle, get(), get(), get(), get(), get(), get(), get(), get(), get(), get(), get(), get(), get())
    }
    viewModel { (courseId: String, courseTitle: String) ->
        ContentTabViewModel(courseId, courseTitle, get(), get())
    }
    viewModel { (courseId: String, courseTitle: String) ->
        CourseHomeViewModel(courseId, courseTitle, get(), get(), get(), get(), get(), get(), get(), get(), get(), get(), get(), get(), get(), get())
    }
    viewModel { (courseId: String) ->
        CourseSectionViewModel(courseId, get(), get(), get(), get())
    }
    viewModel { (courseId: String, unitId: String, mode: CourseViewMode) ->
        CourseUnitContainerViewModel(courseId, unitId, mode, get(), get(), get(), get(), get(), get(), get())
    }
    viewModel { (courseId: String) ->
    ...
    viewModel { (courseId: String, videoUrl: String, blockId: String) ->
        VideoUnitViewModel(courseId, videoUrl, blockId, get(), get(), get(), get(), get(), get())
    }
    viewModel { (courseId: String, enrollmentMode: String) ->
        CourseDatesViewModel(
            courseId = courseId, enrollmentMode = enrollmentMode, courseNotifier = get(),
            interactor = get(), courseAnalytics = get(), config = get(),
            calendarInteractor = get(), calendarNotifier = get(), corePreferences = get(),
            resourceManager = get(),
        )
    }
    viewModel { (courseId: String, handoutsType: String) ->
        HandoutsViewModel(courseId, handoutsType, config = get(), interactor = get(), courseAnalytics = get(), resourceManager = get())
    }
    ...
    viewModel { (courseId: String, courseTitle: String) ->
        DiscussionTopicsViewModel(courseId, courseTitle, interactor = get(), resourceManager = get(), analytics = get(), courseNotifier = get())
    }
    viewModel { (courseId: String, topicId: String, threadType: String) ->
        DiscussionThreadsViewModel(get(), get(), get(), courseId, topicId, threadType)
    }
    viewModel { (thread: org.openedx.discussion.domain.model.Thread) ->
        DiscussionCommentsViewModel(get(), get(), get(), thread)
    }
    viewModel { (comment: DiscussionComment) ->
        DiscussionResponsesViewModel(get(), get(), get(), comment)
    }
    ...
    viewModel { (courseId: String?, infoType: String?) ->
        WhatsNewViewModel(courseId, infoType, get(), get(), get(), get(), get())
    }
    ...
    viewModel {
        ProgramViewModel(appData = get(), config = get(), networkConnection = get(), notifier = get(), edxCookieManager = get(), resourceManager = get(), interactor = get())
    }

    viewModel { (courseId: String, courseTitle: String) ->
        CourseOfflineViewModel(courseId, courseTitle, get(), get(), get(), get(), get(), get(), get(), get(), get(), get(), get())
    }
    viewModel { (courseId: String) -> CourseProgressViewModel(courseId, get(), get(), get()) }

    single { DownloadRepositoryImpl(api = get(), corePreferences = get(), dao = get(), courseDao = get()) as DownloadRepository }
    single { DownloadInteractor(repository = get()) }
    viewModel {
        DownloadsViewModel(
    ...
    viewModel { (courseId: String) ->
        CourseAssignmentViewModel(courseId = courseId, interactor = get(), courseNotifier = get(), analytics = get())
    }

    factory { DatesRepositoryImpl(api = get(), dao = get(), preferencesManager = get()) as DatesRepository }
    factory { DatesInteractor(repository = get()) }
    viewModel {
        DatesViewModel(
