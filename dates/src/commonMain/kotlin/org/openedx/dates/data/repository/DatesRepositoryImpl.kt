...
class DatesRepositoryImpl(
    ...
    override suspend fun getUserDates(page: Int): CourseDatesResponse {
    ...
    override suspend fun getUserDatesFromCache(): List<CourseDate> {
        return dao.getCourseDates().mapNotNull { it.mapToDomain() }
    }

    override suspend fun preloadFirstPageCachedDates(): List<CourseDate> {
        return dao.getCourseDates(PAGE_SIZE).mapNotNull { it.mapToDomain() }
    }

    override suspend fun shiftAllDueDates() = api.shiftAllDueDates()
