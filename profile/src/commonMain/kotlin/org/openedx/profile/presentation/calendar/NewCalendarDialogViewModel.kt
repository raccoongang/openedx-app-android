...
import org.openedx.core.core_error_unknown_error
import org.openedx.core.core_error_no_connection
...
import org.openedx.foundation.extension.NoConnectionException
    ...
    fun createCalendar(
        ...
        viewModelScope.launch {
            if (networkConnection.isOnline()) {
                ...
                handleErrorUiMessage(
                    throwable = NoConnectionException(),
                )
            }
        }
    }
            ...
            if (!networkConnection.isOnline()) {
                sendMessage(
                    UIMessage.SnackBarMessage(
                        resourceManager.getString(coreRes.string.core_error_no_connection)
                    )
                )
                return@launch
            }
                ...
                sendMessage(
                    UIMessage.SnackBarMessage(
                        resourceManager.getString(
                            coreRes.string.core_error_unknown_error
                        )
                    )
                )
                return@launch
