...
class HtmlUnitViewModel(
    ...
    private val jsInjectionProvider: JsInjectionProvider,
    ...
    fun setWebPageLoaded() {
        ...
        jsInjectionProvider.getCompletionsJs()?.let { jsList.add(it) }
        // Injection to fix CSS issues for Survey xBlock
        jsInjectionProvider.getSurveyCssJs()?.let { jsList.add(it) }
    ...
    private fun getXBlockProgress() {
        viewModelScope.launch {
            if (!isOnline) {
                val jsonProgress = courseInteractor.getXBlockProgressJson(blockId)
                delay(500)
                _uiState.value = HtmlUnitUIState.Loaded(jsonProgress = jsonProgress)
            }
        }
    }
}
