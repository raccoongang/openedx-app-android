...
import org.openedx.foundation.utils.saveObjectToFile
...
class DashboardRepositoryImpl(
    ...
    override suspend fun getEnrolledCourses(page: Int): DashboardCourseList {
    ...
    override suspend fun getEnrolledCoursesFromCache(): List<EnrolledCourse> {
        val list = dao.readAllData()
        return list.map { it.mapToDomain() }
    }

    override suspend fun getMainUserCourses(pageSize: Int): CourseEnrollments {
    ...
    override suspend fun getAllUserCourses(page: Int, status: CourseStatusFilter?): DashboardCourseList {
        val user = preferencesManager.user
        val result = api.getUserCourses(
            username = user?.username ?: "",
            page = page,
